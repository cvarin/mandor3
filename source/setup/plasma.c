...
plasma_marker (void)
{
    ...
    if (plasma->i >= plasma->N) {
        ...
        ENSURE (plasma->p, "cannot grow storage up to %d particles",
                              plasma->N);
    }
...
// ---------------------------------------------------------------------------
/// Returns object number 'count' counting from last to the first object.
// ---------------------------------------------------------------------------
marker_t*
...
// ---------------------------------------------------------------------------
/// Saves all markers into file with given name.
// ---------------------------------------------------------------------------
void
plasma_save (const char *name)
{
    ...
    // Dummy writing to reserve space in file.
